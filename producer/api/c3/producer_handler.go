...
import (
	...
	c3 "service/producer/demo/internal/grpc/c3"
...
func SendDataServiceConsumer3(ctxF *fiber.Ctx) error {

	grpc_host, grpc_port, grpc_tls := config.GRPCConfig3()
	...
	go func() {
		defer wg.Done()
		c3.StreamConsumerClient3(c, requestChan)
	}()
	requestChan <- request
	close(requestChan)
