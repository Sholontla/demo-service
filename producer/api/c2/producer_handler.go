...
import (
	...
	"service/producer/demo/internal/grpc/c2"
...
func SendDataServiceConsumer2(ctxF *fiber.Ctx) error {

	grpc_host, grpc_port, grpc_tls := config.GRPCConfig2()
	...
	go func() {
		defer wg.Done()
		c2.StreamConsumerClient2(c, requestChan)
	}()
	requestChan <- request
	close(requestChan)
