...
import (
	...
	c4 "service/producer/demo/internal/grpc/c4"
...
func SendDataServiceConsumer4(ctxF *fiber.Ctx) error {

	grpc_host, grpc_port, grpc_tls := config.GRPCConfig4()
	...
	go func() {
		defer wg.Done()
		c4.StreamConsumerClient4(c, requestChan)
	}()
	requestChan <- request
	close(requestChan)
